...
$delta = 1 * 60 * 60; //--------------интервал паузы между поисками
...
////Определяем время последнего поиска берем запись из результатов
////Капча на одной версии google приминяется к другой,
////поэтому время доступа по идее нужно брать меньшее из выборок для разных версий google
//$Domen = array('.ru', '.com');
//foreach ($Domen as $val) {
//
//    $first = $last ? $last : 0;
//    $value = $seacher . $val;
//    $query = "SELECT accessdate FROM $result_tablename WHERE seacher = '$value' ORDER BY accessdate DESC";
//    $result = mysql_query($query);
//
//    if (!$result) {
//        print(mysql_error());
...
//    $query_data = mysql_fetch_array($result);
//
//    $last = intval(strtotime($query_data['accessdate']));
//    $accessdate = $last > $first ? $last : $first;
//}
...
//    die("Работа сервиса приостановлена на $PauseC из-за блокировки Google");
